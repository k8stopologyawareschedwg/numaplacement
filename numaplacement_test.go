...
func TestPayloadValidateSharedChecks(t *testing.T) {
	noopValidate := func(pl Payload) error { return nil }
	tests := []struct {
		name    string
		pl      Payload
		wantErr error
		...
		{
			name: "vectors on zero containers",
			pl: Payload{
				NUMANodes: 2,
				Vectors:   map[int]string{0: "x"},
			},
			wantErr: ErrInconsistentNUMAVectors,
		},
		{
			...
			pl: Payload{
				Containers: 4,
				NUMANodes:  1,
				Vectors:    map[int]string{0: "a", 1: "b"},
			},
			wantErr: ErrInconsistentNUMAVectors,
		},
		{
			name: "vector key negative",
			pl: Payload{
				Containers: 4,
				NUMANodes:  2,
				Vectors:    map[int]string{-1: "x"},
			},
			wantErr: ErrCorruptedNUMAVector,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotErr := tt.pl.Validate(noopValidate)
			if !errors.Is(gotErr, tt.wantErr) {
				t.Errorf("got error = %q, want = %q", gotErr, tt.wantErr)
			}
		})
