...
func (enc *Encoder) Result() (numaplacement.Payload, error) {
	...
	busiestNodeCount := 0
	numaNodes := numaplacement.SortedKeys(vecs)
...
func NewDecoder(pl numaplacement.Payload, ids ...numaplacement.ContainerID) (*Decoder, error) {
	if err := pl.Validate(VectorEncodingPlain); err != nil {
		return nil, err
	}
	dec := &Decoder{
		payload: pl,
...
func (dec *Decoder) Result() (numaplacement.Info, error) {
	...
	for cid := range dec.cntSet {
		if _, ok := info.numaLocality[cid]; !ok {
			info.numaLocality[cid] = dec.payload.BusiestNode
		}
	}
	return info, nil
}
